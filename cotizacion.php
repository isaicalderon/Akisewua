<?php 
	...
	if (($id = @$_GET["id"]) != "") {
		$edit = true;
		$token = lexico($id);
		$query = mysqli_query($con, "SELECT p.ID, p.ID_User, p.ID_Prod, p.Descripcion, c.Nombres, c.Apellidos, p.status, p.Fecha, pp.alto FROM clientes c, cotizaciones p, productos pp WHERE pp.ID = p.ID_Prod AND c.ID = p.ID_User AND p.ID = ".$token)or die(mysqli_error($con));
		...
		<main role="main">
			<div class="container" style="margin-top: 2%">
				<div class="row">
					...
					<div class="col-md-8">
						<form class="needs-validation" novalidate method="post" action="php/aceptarPedido.php" enctype="multipart/form-data">
							<div class="row">
								<div class="col-md-6 mb-3">
									<label>Descripcion: <br><?php echo $row['Descripcion']; ?></label>
							...
							<input type="texto" name="id" <?php echo "value='".$row['ID']."' "; ?> style="display: none">
