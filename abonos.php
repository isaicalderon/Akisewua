<div class="col-md-10">
		...
		<table class="tablaAlum">
			...
			<tbody>
				<?php 
					...
					$query = "SELECT c.Nombres, c.Apellidos, coti.ID_Prod,pp.Concepto_venta, pp.Monto_inicial, pp.Fecha_final FROM pedidos_proceso pp, cotizaciones coti, clientes c WHERE pp.ID_Coti = coti.ID AND coti.ID_User = c.ID AND pp.status = 0 ".$tmp." ORDER BY pp.ID DESC";
				 	...
				 	while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
						...
						echo "
			 			<tr>
			 				...
			 				<td>".$row['Monto_inicial']." USD</td>
		 					<td>".$row['Fecha_final']."</td>
		 				</tr>
				 		";
				 		$money += $row['Monto_inicial'];
				 	}
				 	echo "
				 	<script>
	...
	<?php
		echo "
		<label class='btn btn-primary' onclick=\"window.location.href='sql.php?option=2&data=".@$_GET['data']."&mon=".$money."&value=".@$_GET['value']."&date=".urlencode (@$_GET['date']).";' \" >Imprimir</label>
		";
	?>
</div>